...
public class SuggestionEngine extends Java8BaseListener {
	...
	public void enterMethodDeclaration(
		...
		if (ctx.methodModifier(0) != null)
			modifier = ctx.methodModifier(0).getText();// returns method modifier as string
		if (modifier != null && modifier.contains("public")) {
			String name = ctx.methodHeader().methodDeclarator().getText();
	...
	private TreeSet<Candidate> getTopKNeighbor(String word, int K) {
		...
		for (String s : mMethods) {

			double distance = Levenshtein.distance(word, s);
			Candidate c = new Candidate(s, distance);
			minHeap.add(c);
